...
let irysConfig;
let activeWalletSigner; // To store the active ethers Wallet signer
let activeRpcProvider; // To store the active ethers JsonRpcProvider
let activeWalletAddress; // To store the address of the active wallet
...
if (IRYS_ACTIVE_NET === 'devnet') {
    ...
    try {
        activeRpcProvider = new ethers_1.JsonRpcProvider(IRYS_DEVNET_RPC_URL);
        const rawWallet = new ethers_1.Wallet(IRYS_DEVNET_EVM_PRIVATE_KEY);
    ...
    try {
        activeRpcProvider = new ethers_1.JsonRpcProvider(IRYS_MAINNET_RPC_URL);
        const rawWallet = new ethers_1.Wallet(IRYS_MAINNET_EVM_PRIVATE_KEY);
        activeWalletSigner = rawWallet.connect(activeRpcProvider);
        activeWalletAddress = activeWalletSigner.address;
        irysConfig = {
...
app.post('/api/initiateSponsoredUpload', async (req, res, next) => {
    try {
        ...
        const irys = new sdk_1.default(irysConfig);
        await irys.ready(); // Ensure SDK is ready
        // For this POC, we grant a fixed approval amount each time.
        // In a production system, you'd check existing allowance and top up intelligently.
        const approvalAmount = DEFAULT_APPROVAL_AMOUNT_ATOMIC;
        // You might want to use irys.utils.toAtomic() if you define amounts in standard units
        // const approvalAmountInStandard = "0.01"; // e.g., 0.01 ETH/MATIC
        // const atomicAmount = irys.utils.toAtomic(approvalAmountInStandard, irys.token); 
        console.log(`[server /api/initiateSponsoredUpload] Approving ${userAddress} for ${approvalAmount} atomic units of ${irys.token} on behalf of sponsor ${SPONSOR_ADDRESS}`);
        const approvalReceipt = await irys.approval.createApproval({
            approvedAddress: userAddress,
            amount: approvalAmount, // Must be in atomic units
            // expiresInSeconds: 3600 // Optional: approve for 1 hour
        });
        console.log(`[server /api/initiateSponsoredUpload] Approval successful for ${userAddress}. Receipt:`, approvalReceipt);
        res.status(200).json({
            success: true,
            sponsorAddress: SPONSOR_ADDRESS,
            message: `User ${userAddress} approved to spend ${approvalAmount} atomic units.`,
            approvalTxId: approvalReceipt.id, // Irys returns a receipt for the approval itself
...
app.post('/api/fundIrysNode', async (req, res, next) => {
    try {
        ...
        if (irys.token === 'arweave') {
            ...
            amountInAtomicUnitsStr = (0, ethers_1.parseUnits)(amountToFundStr, decimals).toString();
        }
...
app.get('/api/irysBalance', async (req, res, next) => {
    try {
...
const checkBalanceAtStartup = async () => {
    try {
        ...
        if (activeWalletSigner && activeRpcProvider) {
            const anEvmBalance = await activeRpcProvider.getBalance(activeWalletAddress);
            console.log(`[server]: Server EVM Wallet (${activeWalletAddress}) balance on ${irysConfig.config?.providerUrl}: ${(0, ethers_1.formatUnits)(anEvmBalance, 18)} ${irysConfig.token === 'ethereum' ? 'ETH' : irysConfig.token}`);
        }
        else {
